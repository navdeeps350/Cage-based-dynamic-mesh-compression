...
import trimesh
import numpy as np
from utils_numba import compute_mvc, mvc_3d_single_into
...
alpha = 0.75 # This can be adjusted as needed
...
if not os.path.exists(f"cage_avg/{action}_cage_avg"):
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith(".obj"):
            ...
            new_mesh = trimesh.Trimesh(vertices=new_vertices, faces=cage_faces)
...
for i in range(mesh_vertices_c.shape[0]):
    ...
    refined_p = refine_vertex_point_newton_method_numba_fast(
        p0, U_seq_c, V_i,
        cage_V_c, cage_F_c,
        steps=1, alpha=1e-6, h=1e-7, mvc_eps=1e-8
    )
...
if not os.path.exists(output_folder):
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith(".obj"):
            obj_path = os.path.join(input_folder, filename)
            # print(obj_path)
            # replace the word "cage" with "mesh" in the filename "dataset_off/decoded/handstand_restructured_obj/cage_00160.obj" and remove first 0 after  underscore
            cage_vertices, cage_faces = igl.read_triangle_mesh(obj_path)
            # print(cage.vertices.shape)
            # print(mvc.shape)
            new_vertices = mvc @ cage_vertices
            # print(new_vertices.shape)
            new_mesh = trimesh.Trimesh(vertices=new_vertices, faces=mesh_faces)
            if not os.path.exists(output_folder):
                os.makedirs(output_folder)
            new_obj_path = os.path.join(output_folder, filename.replace("cage", "mesh").replace("_0", "_"))
            new_mesh.export(new_obj_path, file_type='obj')
            print(f"Transformed {filename} and saved to {new_obj_path}")
...
def save_run_times(a, b, c, d, e, f, g, h, action, alpha):
    df = pd.DataFrame({
        'Action': [action],
        'Alpha': [alpha],
        'a': [a],
...
save_run_times(a, b, c, d, e, f, g, h, action, alpha)
